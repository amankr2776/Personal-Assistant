...
import { Cloud, CloudOff, Minus, Square, X, Zap, Cpu, Wifi, WifiOff, LogOut, User } from 'lucide-react';
...
export default function TopBar() {
  ...
  const modelLabel =
    ...
    activeModel === 'phi3:mini' ? 'Phi-3 Mini' :
    activeModel === 'llama-3.1-8b-instant' ? 'Llama 3.1 Instant' : activeModel;
    ...
    >
      ...
      <div className="flex items-center gap-3" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-heading font-bold text-sm text-jarvis-text tracking-wider">AIRA</span>
        </div>
          ...
          {ollamaOk ? (
            ...
            <>
              ...
              <motion.div
                ...
                transition={{ duration: 2, repeat: Infinity }}
              />
              <span className="text-[10px] text-jarvis-muted font-mono">Gemini AI</span>
            </>
          ) : (
            <>
      ...
      <div className="flex items-center gap-3" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        <div className="flex items-center gap-2 bg-jarvis-bg border border-jarvis-border rounded-md px-2.5 py-1">
          ...
          <span className="text-[10px] font-mono text-jarvis-text">{cloudOk && !ollamaOk ? 'Gemini Flash' : modelLabel}</span>
        </div>
        ...
        {user && (
          <div className="flex items-center gap-2 ml-2">
            ...
            <button
              onClick={logout}
              className="text-[10px] text-jarvis-muted hover:text-red-400 flex items-center gap-0.5 transition-colors"
              title="Sign out"
            >
              <LogOut size={10} />
            </button>
